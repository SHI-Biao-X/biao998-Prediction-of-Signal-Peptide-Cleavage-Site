...
private:
	...
	double kernel_substitution_matrix(int i, int j) const
	{
		...
		int k = 0;
		while(x[i][k].index != -1 && x[j][k].index != -1)
		{
			sum += matrix.GetScore(matrix.GetLetterPosition(x[i][k].index % matrix.GetNbrAlphabet()), matrix.GetLetterPosition(x[j][k].index % matrix.GetNbrAlphabet()));
			++k;
		}
		return sum;
	}
			  ...
			  const svm_parameter& param)
{
	switch(param.kernel_type)
	{
		...
		case SUBMATRIX:
		{
			...
			SubstitutionMatrix matrix = SubstitutionMatrix("../BLOSUM62.txt");
			int k = 0;
			while(x[k].index != -1 && y[k].index != -1)
			{
				sum += matrix.GetScore(matrix.GetLetterPosition(x[k].index % matrix.GetNbrAlphabet()), matrix.GetLetterPosition(y[k].index % matrix.GetNbrAlphabet()));
				++k;
			}
			return sum;
		}
